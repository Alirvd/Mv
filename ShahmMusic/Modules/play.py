...
async def play(_, message: Message):
    Shahm = await message.reply_text("** اوكف نتضر ثواني اي خلل راسلني(@v_vnn)...**")
    try:
    ...
    try:
        try:
            ...
            return await Shahm.edit_text(
                f"  ** ارفوعني اشراف ونطوني اضافه مشرفين حتى اضيف الحساب المساعد ** {BOT_NAME} ** وحاول مرة اخرى {message.chat.title}."
            )
